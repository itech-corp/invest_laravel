    ...
    return (
        ...
        <Col className="mt-3" xs={3}>
            <div style={{ backgroundColor: invest ? '#73EFC2' : '#EFECEC', fontFamily: 'Bahnschrift' }} className="h-100 text-left p-2 rounded-sm font-weight-lighter">
                ...
                <div className="pt-2 text-truncate">Bal W{props.week}: <strong>${props.balPrevW.toFixed(2)} + <span style={{ color: '#039B54' }}>${props.payout.reduce((acc,val)=>Number(acc)+Number(val))}</span></strong></div>
